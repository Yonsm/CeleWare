...
IPAResult IPAInstall(NSString *path)
{
	...
	if (lib)
	{
		...
		if (pMobileInstallationInstall)
		{
			...
			ret = (IPAResult)pMobileInstallationInstall(temp, [NSDictionary dictionaryWithObject:@"User" forKey:@"ApplicationType"], 0, path);
			[[NSFileManager defaultManager] removeItemAtPath:temp error:nil];
			return ret;
		}
		dlclose(lib);
...
NSDictionary *IPAInstalledApps()
{
	...
	if (lib)
	{
		...
		if (pMobileInstallationLookup)
		{
			...
			ret = pMobileInstallationLookup(params, NULL);
			_LogObj(ret);
		}
	}
